...
export class PrefixNotFoundException extends Error {
  constructor(public prefixName: string) {
    super(
      `Prefix ${JSON.stringify(prefixName)} not found inside the configuration. Please add it before adding any related custom mapping.`
    );
...
export class ShortenStrictException extends Error {
  constructor(public namespace: string) {
    super(
      `Namespace ${JSON.stringify(namespace)} not found inside the configuration. Please add it if you want to use the SHORTEN mode.`
    );
...
export class WrongAuthenticationException extends Error {
  constructor(public paramName: string) {
    super(
      `Missing ${paramName} key inside the authentication definition. Remember that it should contain the following keys: [uri, database, user, pwd]`
    );
...
export const NEO4J_DRIVER_MULTIPLE_TYPE_ERROR_MESSAGE =
